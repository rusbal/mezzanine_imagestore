...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding unique constraint on 'Album', fields ['user', 'name']
        db.create_unique(u'imagestore_album', ['user_id', 'name'])

        # Deleting field 'Image.album'
        db.delete_column(u'imagestore_image', 'album_id')

        # Adding M2M table for field albums on 'Image'
        m2m_table_name = db.shorten_name(u'imagestore_image_albums')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('image', models.ForeignKey(orm['imagestore.image'], null=False)),
            ('album', models.ForeignKey(orm['imagestore.album'], null=False))
        ))
        db.create_unique(m2m_table_name, ['image_id', 'album_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'Album', fields ['user', 'name']
        db.delete_unique(u'imagestore_album', ['user_id', 'name'])

        # Adding field 'Image.album'
        db.add_column(u'imagestore_image', 'album',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=0, related_name='images', to=orm['imagestore.Album']),
                      keep_default=False)

        # Removing M2M table for field albums on 'Image'
        db.delete_table(db.shorten_name(u'imagestore_image_albums'))
    ...
    models = {
        ...
        'imagestore.image': {
            ...
            'albums': ('django.db.models.fields.related.ManyToManyField', [], {'related_name': "'images'", 'symmetrical': 'False', 'to': "orm['imagestore.Album']"}),
